...
import com.example.project.basic_api.ui.view.main.rekomendasi.RekomFragment
...
import com.example.project.basic_recycleview.ItemModel
...
class HomeFragment : Fragment() {
    ...
    private fun setupApiNewsHorizonatal(binding: FragmentHomeBinding){
...
//
    ...
    private fun setupGridMenu(binding : FragmentHomeBinding){
        ...
        binding.includedGridMenuHome.cardMenu2.setOnClickListener()  {
            val panduanCVFragment = PanduanCVFragment()
            val transaction = parentFragmentManager.beginTransaction()
            transaction.replace(R.id.fragment_container, panduanCVFragment)
            transaction.addToBackStack(null)
            transaction.commit() }

        binding.includedGridMenuHome.cardMenu3.setOnClickListener() {
            val interview = InterviewFragment()
            val transaction = parentFragmentManager.beginTransaction()
            transaction.replace(R.id.fragment_container, interview)
            transaction.addToBackStack(null)
            transaction.commit() }
